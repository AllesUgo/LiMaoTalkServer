...
const RbsLib::Buffer & RbsLib::Streams::StringStream::Read(Buffer & buffer, int64_t size)
{
	if (size == 0||size>=this->str.length()-this->start)
	{
		...
		{
			buffer.SetData(this->str.c_str() + this->start, buffer.GetSize());
			this->start += buffer.GetSize();
		}
		else
		{
			buffer.SetData(this->str.c_str() + this->start, this->str.length() - this->start);
	...
	else
	{
		...
		{
			buffer.SetData(this->str.c_str() + this->start, size);
			this->start += size;
		}
		else
		{
